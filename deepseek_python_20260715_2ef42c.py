...
WEBAPP_URL = 'https://YOUR_USERNAME.github.io/telegram-dating-bot/webapp/'  # ЗАМЕНИТЕ!
...
class UserStorage:
    ...
    def cleanup_inactive(self):
        """Очистка неактивных пользователей (старше 5 минут)"""
        now = datetime.now()
        to_remove = []
...
async def handle_find_action(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    if storage.is_in_chat(user_id):
        await send_webapp_response(user_id, {
            'action': 'error',
            'text': 'Вы уже в чате'
        })
        return
    ...
    if partner_id is None:
        ...
        await send_webapp_response(partner_id, {
            'action': 'chat_started',
            'message': 'Собеседник найден!'
        })
...
async def handle_next_action(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_partner is None:
        ...
        await send_webapp_response(user_id, {
            'action': 'chat_started',
            'message': 'Новый собеседник найден!'
        })
        await send_webapp_response(new_partner, {
            'action': 'chat_started',
            'message': 'Новый собеседник найден!'
        })
...
async def handle_status_action(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    partner = storage.get_chat_partner(user_id)
...
async def send_webapp_response(user_id: int, data: dict):
    """Отправляет данные в мини-приложение пользователя"""
    try:
